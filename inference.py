...
def get_task_emb(args):
    prompt = args['prompt_name']
    if 'Pick_the_pumpkin' in prompt:
        task_emb = TEXT_EMBEDDINGS[0]
    # elif 'Pick_two_cubes_using_single_arm' in prompt:
    #     task_emb = TEXT_EMBEDDINGS[1]
    elif 'Pick_two_cubes_with_two_arms_separately' in prompt:
        task_emb = TEXT_EMBEDDINGS[2]
    elif 'Play_the_chess' in prompt:
        task_emb = TEXT_EMBEDDINGS[3]
    else:
        print(f'SINGLE TASK embedding wont be used for {prompt}')
    print(f"prompt name :{prompt}")
    assert len(task_emb) == 384
...
def inference_process(args, config, ros_operator, policy, stats, t, pre_action, task_emb):
    ...
    while True and not rospy.is_shutdown():
        ...
        all_actions = policy(curr_image, curr_depth_image, qpos, task_emb=task_emb)
        end_time = time.time()
...
def model_inference(args, config, ros_operator, task_emb, save_episode=True):
    global inference_lock
    global inference_actions
    global inference_timestep
                    ...
                    if t >= max_t:
                        ...
                        inference_thread = threading.Thread(target=inference_process,
                                                            args=(args, config, ros_operator,
                                                                  policy, stats, t, pre_action, task_emb))
...
def get_arguments():
    ...
    parser.add_argument('--use_depth_image', action='store', type=bool, help='use_depth_image',
                        default=False, required=False)
    parser.add_argument('--prompt_name', type=str, default='Pick_the_pumpkin', help=('prompt name for multitask'))
...
def main():
    ...
    task_emb = get_task_emb(args)
    model_inference(args, config, ros_operator, task_emb, save_episode=True)
